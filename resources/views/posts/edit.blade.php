...
@section('posts')
    ...
    <form method="post" action="/posts/{{ $article->getKey() }}">
        ...
        <input type="text" id="inputEmail" class="form-control" name="title" value="{{ old('type', $article->title) }}" minlength="5"
               maxlength="100">
        ...
        <textarea id="inputPassword" class="form-control" name="description" rows="3" maxlength="255">{{ old('type', $article->description) }}</textarea>
        <label for="inputPassword">Содержание:</label>
        <textarea id="inputPassword" class="form-control" name="content" rows="10">{{ old('type', $article->content) }}</textarea>
